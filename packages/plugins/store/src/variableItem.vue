<script setup lang="ts">
  ...
  import EditVariable from './editStoreVariable.vue'
  ...
  const update = () => {
    ...
    if (project.value.store) {
      const idx = project.value.store.findIndex((item: Variable) => item.name === props.variable.name)
      if (idx > -1) {
        project.value.store[idx] = currentState.value
      } else {
        project.value.store.push(currentState.value)
      }
    }
    currentState.value = null
  }
  const doDel = () => {
    ...
    if (project.value.store) {
      const idx = project.value.store.findIndex((item: Variable) => item.name === props.variable.name)
      if (idx > -1) {
        project.value.store.splice(idx, 1)
      }
    }
  }
</script>
